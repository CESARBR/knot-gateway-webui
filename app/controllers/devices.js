...
var get = function get(req, res, next) {
  ...
  users.getUserByUUID(req.user.uuid, function onUser(userErr, user) {
    if (userErr) {
      ...
      devicesSvc.getDevice(req.params.id, function onDeviceGetted(getDeviceErr, serviceDevice) {
        if (getDeviceErr) {
          next(getDeviceErr);
        } else {
          fogSvc.getDevice(user, serviceDevice.uuid, function onDeviceReturned(fogErr, fogDevice) {
            if (fogErr) {
              next(fogErr);
            } else if (!fogDevice) {
              res.sendStatus(404);
            } else {
              fogSvc.getDeviceData(user, serviceDevice.uuid, function onDeviceDataReturned(fogErr2, fogDeviceData) { // eslint-disable-line max-len
                var device;
                if (fogErr2) {
                  next(fogErr);
                } else {
