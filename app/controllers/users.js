...
var create = function create(req, res, next) {
  cloud.getCloudSettings(function onCloudSettings(getCloudErr, cloudSettings) {
    ...
    if (getCloudErr) {
      ...
      cloudSvc = new CloudService(cloudSettings.hostname, cloudSettings.port, cloudSettings.platform); // eslint-disable-line max-len
      if (cloudSvc.platform === 'MESHBLU') {
        signupMeshblu(credentials, cloudSvc, function onsignup(errSignup) {
          if (errSignup) {
            next(errSignup);
          } else {
            res.end();
      ...
      } else if (cloudSvc.platform === 'FIWARE') {
        signupFiware(credentials, function onsignup(errSignup) {
          if (errSignup) {
            next(errSignup);
          } else {
            res.end();
